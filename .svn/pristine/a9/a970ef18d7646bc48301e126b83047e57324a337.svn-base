...
public class PlayBackActivity extends Activity implements OnClickListener, SurfaceHolder.Callback {
    ...
    private void initView() {
        ...
        mProgressSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
            ...
            public void onStopTrackingTouch(SeekBar arg0) {
                ...
                mStartTime = track;
                VMSNetSDK.getInstance().startPlayBackOpt(mSurfaceView, mRecordInfo.getSegmentListPlayUrl(), mStartTime, mEndTime, new OnVMSNetSDKBusiness() {
                    @Override
    ...
    private void getCameraInfo() {
        ...
        VMSNetSDK.getInstance().getPlayBackCameraInfo(String.valueOf(mCamera.getId()), mCamera.getSysCode(), new OnVMSNetSDKBusiness() {
            @Override
            public void onFailure() {
    ...
    private void initStorageTypeView(final int[] mRecordPos, final String[] mGuids) {
        ...
        for (int i = 0; i < mRecordPos.length; i++) {
            ...
            rb.setTag(i);
            switch (mRecordPos[i]) {
                case SDKConstant.PlayBackSDKConstant.RECORD_TYPE_NVT:
                    rb.setText(SDKConstant.PlayBackSDKConstant.RECORD_TYPE_NVT_STR);
                    break;

                case SDKConstant.PlayBackSDKConstant.RECORD_TYPE_PU:
                    rb.setText(SDKConstant.PlayBackSDKConstant.RECORD_TYPE_PU_STR);
                    break;

                case SDKConstant.PlayBackSDKConstant.RECORD_TYPE_NVR:
                    rb.setText(SDKConstant.PlayBackSDKConstant.RECORD_TYPE_NVR_STR);
                    break;

                case SDKConstant.PlayBackSDKConstant.RECORD_TYPE_CVM:
                    rb.setText(SDKConstant.PlayBackSDKConstant.RECORD_TYPE_CVM_STR);
                    break;
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        VMSNetSDK.getInstance().setVideoWindowOpt(holder);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.playBackStop:
                //停止回放按钮点击操作
                boolean stopPlayBackOpt = VMSNetSDK.getInstance().stopPlayBackOpt();
                if (stopPlayBackOpt) {
                    stopUpdateTimer();
            ...
            case R.id.playBackPause:
                //暂停、恢复按钮点击操作
                if (!mIsPause) {
                    boolean pausePlayBackOpt = VMSNetSDK.getInstance().pausePlayBackOpt();
                    if (pausePlayBackOpt) {
                    ...
                    boolean resumePlayBackOpt = VMSNetSDK.getInstance().resumePlayBackOpt();
            ...
            case R.id.playBackCapture:
                //抓拍按钮点击操作
                int opt = VMSNetSDK.getInstance().capturePlaybackOpt(FileUtils.getPictureDirPath().getAbsolutePath(),
            ...
            case R.id.playBackRecord:
                //录像按钮点击操作
                if (!mIsRecord) {
                    int recordOpt = VMSNetSDK.getInstance().startPlayBackRecordOpt(FileUtils.getVideoDirPath().getAbsolutePath(), "Video" + System.currentTimeMillis() + ".mp4");
                    switch (recordOpt) {
                    ...
                    VMSNetSDK.getInstance().stopPlayBackRecordOpt();
                    mIsRecord = false;
            ...
            case R.id.playBackRadio:
                //音频按钮点击操作
                if (mIsAudioOpen) {
                    VMSNetSDK.getInstance().stopPlayBackAudioOpt();
                    mIsAudioOpen = false;
                    UIUtils.showToast(PlayBackActivity.this, "关闭音频");
                    mAudioButton.setText("开启音频");
                } else {
                    boolean retAudio = VMSNetSDK.getInstance().startPlayBackAudioOpt();
                    if (!retAudio) {
                        mIsAudioOpen = false;
                ...
                if (isZoom) {
                    mSurfaceView.setOnZoomListener(new OnZoomListener() {
                        ...
                        public void onZoomChange(CustomRect original, CustomRect current) {
                            VMSNetSDK.getInstance().zoomPlayBackOpt(true, original, current);
                        }
                    });
                } else {
                    mSurfaceView.setOnZoomListener(null);
                    VMSNetSDK.getInstance().zoomPlayBackOpt(false, null, null);
                }
                break;
    ...
    private void queryRecordSegment() {
        ...
        VMSNetSDK.getInstance().queryRecordSegment(mCameraInfo, queryStartTime, queryEndTime, mStorageType, mGuid, new OnVMSNetSDKBusiness() {
            @Override
            public void onFailure() {
    ...
    private void updateRemotePlayUI() {
        //获取播放进度
        long osd = VMSNetSDK.getInstance().getOSDTimeOpt();
        if (osd != -1) {
